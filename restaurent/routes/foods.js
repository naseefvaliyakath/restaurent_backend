...
///search Today food by name
router.get("/searchTodayFood", (req, res) => {
  
    try{
      fdName = req.query.fdName;
      con.query(`SELECT * FROM foods WHERE fdName LIKE '%${fdName}%' AND fdIsToday = 'yes' `, (err, results, fields) => {
        if (err) {
          //error log
  ...
  ///search all food by name
router.get("/searchAllFood", (req, res) => {
  
  try{
    fdName = req.query.fdName;
    con.query(`SELECT * FROM foods WHERE fdName LIKE '%${fdName}%' `, (err, results, fields) => {
      if (err) {
        //error log
  ...
  try {
    ...
    id = req.body.id
    console.log("id : ", id);
    con.query("DELETE FROM foods WHERE id = ?", id, (err, results) => {
        if (err) {
                console.log("error: ", err);
                res.json({"error": true,"errorCode":err['code'],"totalSize": 0,"data":null})
          return;
        ...
        res.json({"error": false,"errorCode":"delete successfully","totalSize": 0,"data":null})
      });
